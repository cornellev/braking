...
def detect_directory(
	...
	"""
	weights_path = '/'.join(os.getcwd().split("/")) + weights_path
	img_path = '/'.join(os.getcwd().split("/")) + img_path
	print("IMG PATH: ", img_path)
	dataloader = _create_data_loader(img_path, batch_size, img_size, n_cpu)
	model = load_model(model_path, weights_path)
	img_detections, imgs = detect(
		model, dataloader, output_path, img_size, conf_thres, nms_thres
	)
...
def detect_image(model, image, img_size=416, conf_thres=0.5, nms_thres=0.5):
	...
	:type img_size: int, optional
	:param conf_thres: Object confidence threshold, defaults to 0.5
	:type conf_thres: float, optioimg = np.array(Image.open(image_path))nal
	:param nms_thres: IOU threshold for non-maximum suppression, defaults to 0.5
	:type nms_thres: float, optional
...
def detect(model, dataloader, output_path, img_size, conf_thres, nms_thres):
	...
	for (img_paths, input_imgs) in tqdm.tqdm(dataloader, desc="Detecting"):
		...
		# Get detectionsimport pdb; pdb.set_trace()
		with torch.no_grad():
...
def save_output_image(img, detections, img_size, output_path, classes, detection_no):
	"""Draws detections in output image and stores this.
	...
	plt.gca().xaxis.set_major_locator(NullLocator())
	plt.gca().yaxis.set_major_locator(NullLocator())
	filename = 'latest' + str(detection_no)
	output_path = os.path.join(output_path, f"{filename}.png")
	plt.savefig(output_path, bbox_inches="tight", pad_inches=0.0)
	plt.close()
...
def _create_data_loader(img_path, batch_size, img_size, n_cpu):
	...
	"""
	dataset = ImageFolder(
		img_path, transform=transforms.Compose([DEFAULT_TRANSFORMS, Resize(img_size)])
	)
...
def run():
	...
	parser.add_argument(
		...
		"--weights",
		type=str,
		default="/trainer/weights/yolov3.weights",
	...
	parser.add_argument(
		...
		"--classes",
		type=str,
		default="/trainer/data/coco.names",
	...
	classes = load_classes('/'.join(os.getcwd().split("/")) + args.classes)  # List of class names
