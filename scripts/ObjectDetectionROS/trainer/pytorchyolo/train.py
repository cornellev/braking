...
def use_builtin_dataset(batch_size, n_cpu, multiscale_training=False):
    # KITTI NOT AVAILABLE
    pass
...
def run():
    ...
    absolute_path = os.path.dirname(__file__).split(os.path.sep)
    path_required = os.path.sep.join(absolute_path[:-1])

    data_config = parse_data_config(path_required + "/" + args.data)
    train_path = path_required + "/" + data_config["train"]
    valid_path = path_required + "/" + data_config["valid"]
    class_names = load_classes(path_required + "/" + data_config["names"])
